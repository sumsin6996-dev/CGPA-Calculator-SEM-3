...
interface SubjectInputProps {
    ...
    onLastInputComplete?: () => void; // Callback when last input is filled
}

export interface SubjectInputRef {
    focusFirstInput: () => void;
}

const SubjectInput = forwardRef<SubjectInputRef, SubjectInputProps>(
    ({ subject, onComponentChange, onLastInputComplete }, ref) => {
        const inputRefs = useRef<{ [key: string]: HTMLInputElement | null }>({});

        // Expose focusFirstInput method to parent
        useImperativeHandle(ref, () => ({
            focusFirstInput: () => {
                const firstComponent = subject.components[0];
                if (firstComponent) {
                    const firstInputKey = `${subject.code}-${firstComponent.name}`;
                    const firstInput = inputRefs.current[firstInputKey];
                    if (firstInput) {
                        firstInput.focus();
                    }
                }
        ...
        const handleChange = (component: SubjectComponent, value: string, index: number) => {
            ...
            const isLastInput = index === subject.components.length - 1;

            if (value.length >= maxDigits) {
                if (!isLastInput) {
                    // Move to next input within the same subject
                ...
                } else if (onLastInputComplete) {
                    // This is the last input of the subject, trigger callback to move to next subject
                    setTimeout(() => onLastInputComplete(), 0);
                }
            }
        };
        ...
        const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, index: number) => {
            ...
            if (e.key === 'Enter') {
                ...
                if (!isLastInput) {
                    ...
                    // This is the last input, move to next subject
                    onLastInputComplete();
                }
            }
        };
